...
def process_simulation_folder(folder_path, area):
    """
    Process a single simulation folder and extract all data
    ...
    # Find the CSV file with dipole data
    # Look for CSV files in the folder
    csv_files = list(Path(folder_path).glob('*.csv'))
    if not csv_files:
        print(f"No CSV files found in {folder_path}. Skipping folder.", file=sys.stderr)
        return None
    
    # Use the first CSV file found (assuming cdm_input_*.csv or similar)
    csv_file_path = str(csv_files[0])
    print(f"  Using CSV file: {os.path.basename(csv_file_path)}")
    ...
    pols_files = sorted(Path(folder_path).glob('*.pols'))
...
def main():
    ...
    for folder_path in Path(root_folder_path).glob('*/*/'):
        ...
        if simulation_data is not None:
            ...
            folder_names.append(os.path.basename(folder_path_str))
        else:
            print(f"Skipped folder: {folder_path_str}")
...
Date: {os.path.basename(output_npz).replace('DDA_simulation_data.npz', '')}
