# This code generates input parameter files for simulations of a 
# spatially disordered photonics crystal. The idea is to couple
# the LSPR of the SRR to the Wood's anomaly of the first diffraction order
# of the lattice.
...
def create_chain_lattice(lattice_vector, chain_length):
    """
    Create a 1D chain lattice with given lattice vector and length.
    The chain is centered at the origin and extends symmetrically.
    
    :param lattice_vector: Tuple (dx, dy) defining the lattice vector in meters
    :param chain_length: Number of lattice sites extending in each direction from center
    :return: Arrays of x and y coordinates in meters
    """
    dx, dy = lattice_vector
    
    # Create positions from -chain_length to +chain_length
    positions = np.arange(-chain_length, chain_length + 1)
    
    # Calculate coordinates
    x_coords = positions * dx
    y_coords = positions * dy
...
if __name__ == "__main__":
    ...
    lattices = {
        0: ('chain', create_chain_lattice((0, lattice_spacing), 200)),  # 1D chain along y-axis
